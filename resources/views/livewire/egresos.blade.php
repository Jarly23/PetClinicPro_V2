<div class="container mx-auto p-6 max-w-7xl">
    ...
    <div class="flex flex-col lg:flex-row gap-6 mb-8">
        <div class="flex-1 bg-indigo-50 border border-indigo-300 rounded-lg p-6 shadow-sm">
            ...
            <div class="mt-6 pt-4 border-t border-indigo-200">
                <h4 class="text-lg font-semibold text-indigo-800 mb-3">Comparación con {{ $estadisticas['mes_anterior'] }}</h4>
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        <span>Balance anterior:</span>
                        <span class="font-semibold">${{ number_format($estadisticas['balance_mes_anterior'], 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Diferencia:</span>
                        <span class="font-semibold {{ $estadisticas['tendencia'] === 'positiva' ? 'text-green-600' : 'text-red-600' }}">
                    ...
                    <div class="flex items-center mt-2">
                        ...
                        <span class="ml-2 px-2 py-1 text-xs rounded-full {{ $estadisticas['tendencia'] === 'positiva' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div class="flex items-center space-x-3 w-full sm:w-auto">
            <select wire:model="filtroMes" class="w-52 sm:w-64 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition">
            ...
            <button wire:click="$refresh" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-2 rounded-lg transition shadow-md">
                Filtrar
            </button>
            <button wire:click="limpiarFiltro" title="Limpiar filtro" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-3 py-2 rounded-lg transition shadow-sm">
                ✕
            </button>
        </div>
        <div class="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto mt-4 sm:mt-0">
            ...
            @endcan
            <button wire:click="exportarEgresos" class="w-full sm:w-auto px-6 py-2 font-semibold text-white bg-green-600 hover:bg-green-700 rounded-lg shadow-md transition">
                Exportar CSV
            </button>
        </div>
    </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                ...
                <form wire:submit.prevent="save" class="space-y-5">
                    ...
                    <div class="mt-6 flex justify-end">
                        <button wire:click="closeModal" type="button" class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded-lg transition mr-3">
                            Cancelar
                        </button>
                        <button wire:click="save" type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg transition" wire:loading.attr="disabled">
                            Guardar
                        </button>
                    </div>
                </form>
        ...
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm text-center">
                ...
                <div class="flex justify-center space-x-4">
                    <button wire:click="$set('confirmingDelete', false)" class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded-lg transition">
                        Cancelar
                    </button>
                    <button wire:click="delete" class="bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-lg transition" wire:loading.attr="disabled">
                        Eliminar
                    </button>
                </div>
            </div>
    ...
    <x-dialog-modal wire:model="confirmingDelete">
        ...
        <x-slot name="footer">
            <button wire:click="$set('confirmingDelete', false)" class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded-lg transition mr-3">
                Cancelar
            </button>
            <button wire:click="delete" class="bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-lg transition" wire:loading.attr="disabled">
                Eliminar
                ...
                @forelse ($egresos as $egreso)
                    <tr class="border-b border-indigo-200 hover:bg-indigo-50 transition">
                        ...
                        <td class="p-4 text-sm text-indigo-800 border border-indigo-300 space-x-2">
                            @can('egresos.edit')
                                <button wire:click="edit({{ $egreso->id }})" class="px-3 py-1 bg-blue-500 hover:bg-blue-400 text-white rounded transition">
                                    Editar
                                </button>
                            @endcan
                            @can('egresos.destroy')
                                <button wire:click="confirmDelete({{ $egreso->id }})" class="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded transition">
                                    Eliminar
                                </button>
                            @endcan
                        </td>
