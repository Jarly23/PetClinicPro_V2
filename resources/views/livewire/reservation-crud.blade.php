<div class="space-y-6">
    ...
    @if (session()->has('message'))
        <div class="mb-4 rounded-md bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 p-4 text-sm text-green-800 dark:text-green-200 flex items-center gap-2">
            <svg class="w-5 h-5 text-green-500 dark:text-green-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
            {{ session('message') }}
        </div>
    @endif

    @if (session()->has('error'))
        <div class="mb-4 rounded-md bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 p-4 text-sm text-red-800 dark:text-red-200 flex items-center gap-2">
            <svg class="w-5 h-5 text-red-500 dark:text-red-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            {{ session('error') }}
        </div>
    @endif
    ...
    <x-dialog-modal wire:model="open">
        <x-slot name="title"><span class="text-indigo-700 dark:text-indigo-300 font-bold">Nueva Reserva</span></x-slot>
        <x-slot name="content">
            <form wire:submit.prevent="save" class="space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium">Cliente</label>
                        <div class="rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-2">
    ...
    <!-- 📋 Tabla de Reservas -->
    <div class="overflow-x-auto bg-white dark:bg-gray-900 shadow rounded-xl border border-gray-200 dark:border-gray-700 mt-6">
        <table class="min-w-full text-sm text-left">
            <thead class="bg-gray-100 dark:bg-gray-800">
                <tr>
                    @foreach ([
                        'Cliente', 'Mascota', 'Veterinario', 'Servicio', 'Fecha', 'Estado', 'Acciones'] as $th)
                        <th class="px-4 py-3 font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">{{ $th }}</th>
                    @endforeach
                </tr>
            </thead>
            <tbody>
                @foreach ($reservations as $reservation)
                    <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                        ...
                        <td class="px-4 py-2 flex flex-wrap gap-2 items-center">
                            @can('reservas.edit')
                                <x-buttons.edit wire:click="edit({{ $reservation->id }})" class="!px-2 !py-1" > Editar</x-buttons.edit>
                            @endcan
                            @can('reservas.destroy')
                                <x-buttons.delete wire:click="delete({{ $reservation->id }})" class="!px-2 !py-1" > Eliminar</x-buttons.delete> 
                            @endcan

                            @if ($reservation->status === 'Pending')
                                <button wire:click="confirmReservation({{ $reservation->id }})"
                                    class="btn-yellow px-2 py-1 rounded text-xs font-semibold shadow hover:scale-105 transition">Confirmar</button>
                            @elseif ($reservation->status === 'Confirmed')
                                <x-buttons.create wire:click="startConsultation({{ $reservation->id }})"
                                    class="btn-indigo px-2 py-1 rounded text-xs font-semibold shadow hover:scale-105 transition">Iniciar Consulta</x-buttons.create>
                                <x-buttons.cancel wire:click="cancelReservation({{ $reservation->id }})"
                                    class="btn-red px-2 py-1 rounded text-xs font-semibold shadow hover:scale-105 transition">Cancelar</x-buttons.cancel>
                            @elseif ($reservation->status === 'Completed')
                                <span class="text-green-600 dark:text-green-400 font-semibold">Consulta Completada</span>
                            @else
                                <span class="text-red-600 dark:text-red-400 font-semibold">Reserva Cancelada</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
    ...
    <x-dialog-modal wire:model="openConsultationModal" class="bg-gray-900 text-gray-200">
        ...
        <x-slot name="content" class="bg-gray-900 text-gray-200 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 mb-2">
                <div>
                    ...
                    <input type="text" disabled value="{{ optional(App\Models\Customer::find($customer_id))->name }}"
            ...
            <textarea wire:model="motivo_consulta" placeholder="Motivo de Consulta *"
                class="w-full rounded-md bg-gray-800 text-gray-100 border border-gray-700 p-2 focus:ring-2 focus:ring-indigo-500" rows="3" required></textarea>
            <x-input-error for="motivo_consulta" class="text-red-400" />

            <textarea wire:model="tratamiento" placeholder="Tratamiento (opcional)"
                class="w-full rounded-md bg-gray-800 text-gray-100 border border-gray-700 p-2 focus:ring-2 focus:ring-indigo-500" rows="3"></textarea>
            <x-input-error for="tratamiento" class="text-red-400" />

            <div class="grid grid-cols-2 gap-4">
                <input type="number" wire:model="peso" placeholder="Peso (kg) (opcional)"
                    class="w-full rounded-md bg-gray-800 text-gray-100 border border-gray-700 p-2 focus:ring-2 focus:ring-indigo-500" step="0.01" />
                <input type="number" wire:model="temperatura" placeholder="Temperatura (°C) (opcional)"
                    class="w-full rounded-md bg-gray-800 text-gray-100 border border-gray-700 p-2 focus:ring-2 focus:ring-indigo-500" step="0.1" />
            </div>
        ...
        <x-slot name="footer" class="bg-gray-900 text-gray-200 flex gap-2">
            ...
            <x-buttons.create wire:click.prevent="saveConsultation" class="bg-green-700 hover:bg-green-600" > Guardar Consulta</x-buttons.create>
        </x-slot>
    </x-dialog-modal>
</div>
